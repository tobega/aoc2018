  ...
  OpenPosition withConstraint(Point where, String symbol) {
    ...
    if (where.x == position.x // same row
            ...
            (where.x ~/ 3 == position.x ~/ 3 &&
                where.y ~/ 3 == position.y ~/ 3) // same block
        ) {
...
List<List<String>> placeRemainingDigits(List<OpenPosition> remaining) {
  if (remaining.isEmpty) {
    return List.generate(9, (index) => List.filled(9, '.', growable: false),
        growable: false);
  }
  var bestOpen =
      remaining.reduce((a, b) => b.choices.length < a.choices.length ? b : a);
  remaining.remove(bestOpen);
  while (bestOpen.choices.isNotEmpty) {
    var chosenDigit = bestOpen.choices.removeAt(0);
    var result = placeRemainingDigits(remaining
        .map((e) => e.withConstraint(bestOpen.position, chosenDigit))
        .toList());
    if (result != null) {
...
List<OpenPosition> parseSudoku(String input) {
  ...
  input
      .split('')
      .where((element) => meaningfulInput.contains(element))
      .toList()
      .asMap()
      .forEach((i, s) {
    positions.add(OpenPosition(
        Point(i ~/ 9, i % 9), digits.contains(s) ? [s] : List.of(digits)));
  });
  return positions;
}
...
String solveSudoku(String input) {
  ...
  return rows
          .sublist(0, 3)
          .followedBy(['-' * 11])
          .followedBy(rows.sublist(3, 6))
          .followedBy(['-' * 11])
          .followedBy(rows.sublist(6, 9))
          .join('\n') +
      '\n';
}
