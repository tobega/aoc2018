...
void main() {
  group('internal solver', () {
    test('no more open positions returns 9x9 array of dots', () {
      expect(placeRemainingDigits([]), allOf(
        hasLength(equals(9)),
        everyElement(hasLength(equals(9))),
        everyElement(everyElement(equals('.')))
      ));
    });
    test('last digit gets placed', () {
      expect(placeRemainingDigits([OpenPosition(Point(0, 0), ['5'])]), (result) => result[0][0] == '5');
    });
    test('solves 3 digits on row', () {
      expect(placeRemainingDigits([OpenPosition(Point(0, 0), ['5']),
        OpenPosition(Point(0, 4), ['2', '5', '7']),
        OpenPosition(Point(0, 8), ['2', '5'])
      ]), allOf((result) => result[0][0] == '5',
                (result) => result[0][4] == '7',
                (result) => result[0][8] == '2')
      );
    });
    test('solves 3 digits on column', () {
      expect(placeRemainingDigits([OpenPosition(Point(1, 0), ['6', '7', '9']),
        OpenPosition(Point(5, 0), ['7']),
        OpenPosition(Point(6, 0), ['7', '9'])
      ]), allOf((result) => result[1][0] == '6',
                (result) => result[5][0] == '7',
                (result) => result[6][0] == '9')
      );
    });
    test('solves 3 digits in block', () {
      expect(placeRemainingDigits([OpenPosition(Point(3, 2), ['4', '6']),
        OpenPosition(Point(4, 0), ['6']),
        OpenPosition(Point(5, 1), ['4', '6', '9'])
      ]), allOf((result) => result[3][2] == '4',
                (result) => result[4][0] == '6',
                (result) => result[5][1] == '9')
      );
    });
    ...
    test('contradiction is backtracked', () {
      expect(placeRemainingDigits([OpenPosition(Point(0, 0), ['3', '5']),
  ...
  test('input sudoku', () {
   ...
   '''53.|.7.|...
    ...
    expect(parsedInput[18].choices, equals(['1','2','3','4','5','6','7','8','9']));
